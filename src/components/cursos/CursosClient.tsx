...
/*
 * Descobre o horário de uma turma já cadastrada.
 * Isso permite que turmas antigas continuem aparecendo
 * corretamente na grade.
 */
const getHorarioId = (turma: Partial<Turma>) => {
  if (!turma.data_hora_inicio) return "";
  ...
  const horario = HORARIOS.find((h) => {
    ...
    return (
      h.turno === turma.turno &&
      totalMinutes === horaInicio * 60 + minutoInicio
    );
  });
...
/*
 * Cria as datas de início e fim que serão salvas
 * no banco de dados.
 */
...
export function CursosClient({
  ...
  const handleSaveTurma = async (
    ...
    /*
     * Confere se o horário realmente
     * pertence ao turno selecionado.
     */
    ...
    /*
     * Cria as datas correspondentes
     * ao horário selecionado.
     */
    const {
