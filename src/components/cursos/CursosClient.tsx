...
export function CursosClient({
  ...
  const handleSaveTurma = async (e: React.FormEvent) => {
    ...
    /*
      Mantemos a estrutura original do banco.
    ...
    */
    ...
    const data_hora_inicio = isManha
      ? "2024-01-01T08:00:00.000Z"
      : "2024-01-01T14:00:00.000Z";

    const data_hora_fim = isManha
  ...
  /* =========================================================
     ...
     Corrige diferenças vindas do banco:
     ...
     Todos passam a ser considerados "segunda".
  ========================================================= */
  ...
  const normalizeDia = (value: unknown) => {
    ...
    return aliases[dia] || dia;
  };

  const normalizeTurno = (value: unknown) => {
    const turno = normalizeText(value);
  ...
  /* =========================================================
     ...
     Agora a comparação não depende de:
       - maiúsculas/minúsculas
       - acentos
       - "segunda" x "segunda-feira"
       - "manhã" x "manha"
  ========================================================= */

  const getTurmasForCell = (turno: string, dia: string) => {
    const turnoNormalizado = normalizeTurno(turno);
    const diaNormalizado = normalizeDia(dia);

    return turmas.filter((turma) => {
      const turnoTurma = normalizeTurno(turma.turno);

      const diasTurma = Array.isArray(turma.dias_semana)
        ? turma.dias_semana
        : [];

      const possuiDia = diasTurma.some(
        (diaTurma) =>
          normalizeDia(diaTurma) === diaNormalizado
      );

      return (
        turnoTurma === turnoNormalizado &&
        possuiDia
      );
    });
  };
                  ...
                  {turmasDoCurso.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-2 max-h-[200px] overflow-y-auto pr-1">
                      ...
                      {turmasDoCurso.map((turma) => {
                        ...
                        const turmaColor =
                          COLOR_THEMES[getColorForId(turma.id)];
            ...
            >
              ...
              <div className="pt-4 flex justify-end gap-2 border-t border-slate-100">
                ...
                <button
                  ...
                  disabled={
                    isSaving ||
                    (editingTurma?.dias_semana?.length || 0) === 0
                  }
