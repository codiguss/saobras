...
// Horários disponíveis na grade semanal.
// O horário é salvo no Supabase em UTC, mas sempre é interpretado
// como horário local de Brasília (America/Sao_Paulo) na tela.
const HORARIOS = [
  { id: "manha-1", turno: "Manhã", label: "08:30 - 10:00", inicio: "08:30", fim: "10:00" },
  { id: "manha-2", turno: "Manhã", label: "10:00 - 11:30", inicio: "10:00", fim: "11:30" },
  { id: "tarde-1", turno: "Tarde", label: "14:30 - 16:00", inicio: "14:30", fim: "16:00" },
  { id: "tarde-2", turno: "Tarde", label: "16:00 - 17:30", inicio: "16:00", fim: "17:30" },
] as const;
...
const getLocalTimeFromDate = (value?: string | null) => {
  if (!value) return null;

  // Para valores ISO vindos do PostgreSQL/Supabase, convertemos para
  // o horário de Brasília antes de comparar com os horários da grade.
...
const getHorarioId = (turma: Partial<Turma>) => {
  const localTime = getLocalTimeFromDate(turma.data_hora_inicio);
  ...
  if (localTime) {
    ...
    if (horarioExato) return horarioExato.id;
  }

  // Compatibilidade com turmas antigas que foram salvas como
  // 08:00-12:00 ou 14:00-18:00, antes da criação dos horários.
  if (turma.data_hora_inicio) {
    const date = new Date(turma.data_hora_inicio);

    if (!Number.isNaN(date.getTime())) {
      const localTime = getLocalTimeFromDate(turma.data_hora_inicio);

      if (turma.turno === "Manhã" && localTime === "08:00") {
        return "manha-1";
      }

      if (turma.turno === "Manhã" && localTime === "10:00") {
        return "manha-2";
      }

      if (turma.turno === "Tarde" && localTime === "14:00") {
        return "tarde-1";
      }

      if (turma.turno === "Tarde" && localTime === "16:00") {
        return "tarde-2";
      }
    }
  }
...
const createHorarioDates = (horarioId: string) => {
  ...
  // O offset -03:00 garante que 08:30 na tela continue sendo 08:30
  // em Brasília mesmo depois de o banco converter o valor para UTC.
...
export function CursosClient({ cursos, turmas }: { cursos: Curso[], turmas: Turma[] }) {
  ...
  const handleSaveTurma = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingTurma?.nome || !editingTurma?.curso_id || !editingTurma?.turno) {
    ...
    if (!editingTurma.horario) {
      alert("Selecione um horário para a turma!");
      setIsSaving(false);
      return;
    }

    const horario = getHorarioById(editingTurma.horario);

    if (!horario || horario.turno !== editingTurma.turno) {
      alert("O horário selecionado não corresponde ao turno da turma!");
      setIsSaving(false);
      return;
    }

    const { inicio: data_hora_inicio, fim: data_hora_fim } =
      createHorarioDates(editingTurma.horario);
  ...
  const getTurmasForCell = (turno: string, dia: string, horarioId: string) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          {activeTab === "cursos" ? (
            ...
            <button onClick={() => { setEditingTurma({ nome: "", curso_id: "", turno: "Manhã", dias_semana: [], horario: "manha-1", vagas: 30 }); setIsTurmaModalOpen(true); }} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2">
      ...
      <div className="flex border-b border-slate-200">
        <button onClick={() => setActiveTab("turmas")} className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === "turmas" ? "border-emerald-500 text-emerald-700" : "border-transparent text-slate-500 hover:text-slate-700"}`}>
          <Calendar className="w-4 h-4" /> Grade Semanal
        </button>
        <button onClick={() => setActiveTab("cursos")} className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === "cursos" ? "border-blue-500 text-blue-700" : "border-transparent text-slate-500 hover:text-slate-700"}`}>
      ...
      {activeTab === "cursos" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {cursos.map(curso => {
            ...
            return (
              <div key={curso.id} className={`bg-white border border-slate-200 border-t-4 ${cursoColor.cardBorder} rounded-lg p-5 shadow-sm hover:shadow transition-shadow flex flex-col h-full`}>
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold text-lg text-slate-900">{curso.titulo}</h3>
                  <div className="flex gap-1 shrink-0">
                    <button onClick={() => { setEditingCurso(curso); setIsCursoModalOpen(true); }} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors"><Edit2 className="w-4 h-4" /></button>
                    <button onClick={() => handleDeleteCurso(curso.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
                <p className="text-sm text-slate-500 line-clamp-2 mb-4 flex-grow">{curso.descricao || "Sem descrição"}</p>
                
                <div className="mt-auto pt-4 border-t border-slate-100">
                  <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Turmas Ativas ({turmasDoCurso.length})</h4>
                  {turmasDoCurso.length > 0 ? (
                    <div className="flex flex-col gap-2 max-h-[200px] overflow-y-auto pr-1">
                      {turmasDoCurso.map(turma => {
                        const turmaColor = COLOR_THEMES[getColorForId(turma.id)];
                        return (
                          <div key={turma.id} className={`flex items-center justify-between bg-slate-50 border border-slate-100 border-l-4 ${turmaColor.listCard} p-2.5 rounded-md`}>
                            <div className="flex flex-col min-w-0">
                              <span className="font-medium text-sm text-slate-800 truncate">{turma.nome}</span>
                              <span className="text-[11px] text-slate-500 truncate">{turma.turno} • {(turma.dias_semana || []).join(", ")}</span>
                            </div>
                            <span className={`text-[10px] font-bold px-2 py-1 rounded whitespace-nowrap ml-2 border ${turmaColor.badge}`}>
          ...
          {cursos.length === 0 && <p className="text-slate-500 col-span-full py-8 text-center bg-slate-50 rounded-lg border border-dashed border-slate-300">Nenhum curso cadastrado ainda.</p>}
        </div>
      )}
        ...
        <div className="bg-white border border-slate-200 rounded-lg shadow-sm overflow-x-auto">
          <table className="w-full text-sm text-left min-w-[800px]">
            <thead className="bg-slate-50 text-slate-600 font-bold border-b border-slate-200">
              <tr>
                <th className="px-4 py-4 w-32 border-r border-slate-200 text-center"><Clock className="w-4 h-4 inline-block mr-1"/> Turno</th>
                {DIAS_SEMANA.map(dia => (
                  <th key={dia} className="px-4 py-4 w-48 border-r border-slate-200 text-center">{dia}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {HORARIOS.map((horario) => (
                <tr key={horario.id}>
                  ...
                  {DIAS_SEMANA.map(dia => {
                    const turmasNoDia = getTurmasForCell(horario.turno, dia, horario.id);
                    return (
                      <td key={dia} className="p-1.5 border-r border-slate-200 align-middle bg-white h-[120px]">
                        <div className="w-full h-full flex flex-col gap-1.5">
                          {turmasNoDia.map(turma => {
                            const turmaColor = COLOR_THEMES[getColorForId(turma.id)];
                            return (
                              <div key={turma.id} onClick={() => { setEditingTurma({ ...turma, horario: getHorarioId(turma) }); setIsTurmaModalOpen(true); }} className={`flex-1 flex flex-col items-start justify-center p-3 rounded-md cursor-pointer transition-all text-left border-l-4 ${turmaColor.gridCard}`}>
                                <div className="font-bold text-slate-800 text-base leading-tight mb-1">{turma.nome}</div>
                                <div className={`text-sm font-medium line-clamp-1 mb-2 ${turmaColor.gridTextSub}`}>{turma.cursos?.titulo || "Curso Indefinido"}</div>
                                <div className={`mt-auto inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold border shadow-sm ${turmaColor.badge}`}>
            ...
            <form onSubmit={handleSaveTurma} className="p-4 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input required type="number" min="1" placeholder="Ex: 30" value={editingTurma?.vagas || ""} onChange={e => setEditingTurma(prev => ({...prev, vagas: parseInt(e.target.value) || 0}))} className="w-full px-3 py-2 border border-slate-300 rounded-md focus:ring-emerald-500 focus:border-emerald-500 text-sm"/>
                </div>
              </div>
                ...
                <div className="flex gap-4">
                  {TURNOS.map(t => (
                    <label key={t} className={`flex-1 flex items-center justify-center gap-2 p-3 border rounded-md cursor-pointer transition-colors ${editingTurma?.turno === t ? 'border-emerald-500 bg-emerald-50 text-emerald-800 font-bold' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                      <input type="radio" name="turno" value={t} checked={editingTurma?.turno === t} onChange={() => setEditingTurma(prev => ({
                        ...prev,
                        turno: t,
                        horario: t === "Manhã" ? "manha-1" : "tarde-1"
                      }))} className="hidden" />
                      {t}
                    </label>
                  ))}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {HORARIOS.filter(h => h.turno === editingTurma?.turno).map(horario => (
                    <button
                      key={horario.id}
                      type="button"
                      onClick={() => setEditingTurma(prev => ({ ...prev, horario: horario.id }))}
                      className={`p-3 border rounded-md text-sm font-medium transition-colors ${
                        editingTurma?.horario === horario.id
              ...
              <div className="pt-4 flex justify-end gap-2 border-t border-slate-100">
                ...
                <button type="submit" disabled={isSaving || (editingTurma?.dias_semana?.length || 0) === 0 || !editingTurma?.horario} className="px-4 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-md disabled:opacity-50 flex items-center gap-2">
