...
export function CursosClient({ cursos, turmas }: { cursos: Curso[], turmas: Turma[] }) {
  ...
  const handleSaveTurma = async (e: React.FormEvent) => {
    ...
    const isManha = editingTurma.turno === "Manhã";
    const data_hora_inicio = isManha ? "2024-01-01T08:00:00.000Z" : "2024-01-01T14:00:00.000Z";
    const data_hora_fim = isManha ? "2024-01-01T12:00:00.000Z" : "2024-01-01T18:00:00.000Z";
  ...
  const getTurmasForCell = (turno: string, dia: string) => {
    return turmas.filter(t => t.turno === turno && (t.dias_semana || []).includes(dia));
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          {activeTab === "cursos" ? (
            ...
            <button onClick={() => { setEditingTurma({ nome: "", curso_id: "", turno: "Manhã", dias_semana: [], vagas: 30 }); setIsTurmaModalOpen(true); }} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2">
              ...
              {TURNOS.map(turno => (
                <tr key={turno}>
                  <td className="px-4 py-6 border-r border-slate-200 bg-slate-50 text-center font-bold text-slate-700 align-middle">
                    {turno}<br/>
                    <span className="text-xs font-normal text-slate-500">{turno === "Manhã" ? "08:00 - 12:00" : "14:00 - 18:00"}</span>
                  </td>
                  {DIAS_SEMANA.map(dia => {
                    const turmasNoDia = getTurmasForCell(turno, dia);
                    return (
                      <td key={dia} className="p-1.5 border-r border-slate-200 align-middle bg-white h-[120px]">
                        <div className="w-full h-full flex flex-col gap-1.5">
                          {turmasNoDia.map(turma => {
                            const turmaColor = COLOR_THEMES[getColorForId(turma.id)];
                            return (
                              <div key={turma.id} onClick={() => { setEditingTurma(turma); setIsTurmaModalOpen(true); }} className={`flex-1 flex flex-col items-start justify-center p-3 rounded-md cursor-pointer transition-all text-left border-l-4 ${turmaColor.gridCard}`}>
                ...
                <div className="flex gap-4">
                  {TURNOS.map(t => (
                    <label key={t} className={`flex-1 flex items-center justify-center gap-2 p-3 border rounded-md cursor-pointer transition-colors ${editingTurma?.turno === t ? 'border-emerald-500 bg-emerald-50 text-emerald-800 font-bold' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                      <input type="radio" name="turno" value={t} checked={editingTurma?.turno === t} onChange={() => setEditingTurma(prev => ({...prev, turno: t}))} className="hidden" />
                      {t}
                    </label>
                  ))}
                </div>
