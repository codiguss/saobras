...
export function AlunosClient({ alunos }: { alunos: Aluno[] }) {
  ...
  // Carrega os cursos cadastrados no sistema
  useEffect(() => {
  ...
  // Carrega as matrículas quando abre os detalhes do aluno
  useEffect(() => {
    if (selectedAluno && !isEditing) {
      const fetchMatriculas = async () => {
        ...
        const { data, error } = await supabase
          .from("matriculas")
          .select("*, turmas(nome), cursos(titulo)")
  ...
  // Inicia o cadastro de um novo aluno
  const startCreating = () => {
  ...
  // Inicia a edição de um aluno
  const startEditing = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (selectedAluno) {
      const { data, error } = await supabase
        .from("matriculas")
        .select("id, curso_id, status, data_matricula")
        .eq("aluno_id", selectedAluno.id)
  ...
  // Salva aluno + curso/matrícula
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!cursoId) {
      alert("Selecione um curso antes de salvar o aluno.");
      return;
    }
    ...
    // ==========================================
    // CRIA OU ATUALIZA A MATRÍCULA
    // ==========================================
    if (!dbError && alunoId) {
      const {
        data: matriculaExistente,
        error: matriculaBuscaError,
      } = await supabase
        .from("matriculas")
        .select("id")
        .eq("aluno_id", alunoId)
      ...
      if (matriculaBuscaError) {
        dbError = matriculaBuscaError;
      }

      // Já existe matrícula: apenas altera o curso
      ...
      // Não existe matrícula: cria uma nova
      else {
        const { error } = await supabase
          .from("matriculas")
          .insert([
    ...
    if (dbError) {
      alert(
        "Erro ao salvar: " +
          dbError.message +
          "\n\nVerifique as permissões e as colunas das tabelas alunos, matriculas e cursos no Supabase."
      );
      return;
    }
  ...
  const filteredAlunos = (alunos || []).filter(
    (aluno) => {
      ...
      return (
        ...
        (aluno.cpf?.toLowerCase() || "").includes(
          term
        ) ||
        (
              ...
              {!isEditing && selectedAluno ? (
                <>
                  ...
                  {selectedAluno.tags_perfil &&
                    selectedAluno.tags_perfil.length > 0 && (
